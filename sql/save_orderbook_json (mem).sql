...
-- delete from mem.orderbook
-- select * from [dbo].[orderbook] order by insert_date desc, ts desc
/*
...
begin atomic with (TRANSACTION ISOLATION LEVEL = SNAPSHOT, LANGUAGE = N'us_english')  
	...
	select exchange, pair, ts, dt, 'bid' side, price, amount  --GETUTCDATE() insert_date, [timestamp], ts, @exchange, @pair, price, amount, [type]
		...
		cross apply OPENJSON(@json,'$.orderbook.bids') 
		with (
			price float '$[0]',
			amount float '$[1]'
		) bids
	...
	select exchange, pair, ts, dt, 'ask' side, price, amount  --GETUTCDATE() insert_date, [timestamp], ts, @exchange, @pair, price, amount, [type]
		...
		cross apply OPENJSON(@json,'$.orderbook.asks') 
		with (
			price float '$[0]',
			amount float '$[1]'
		) bids
