...
/*
 ...
 --ALTER TABLE mem.order_book ADD PERIOD FOR SYSTEM_TIME (validFrom, validTill); 
...
*/
	...
	from  
		...
		cross apply (select sid from tokens where symbol=substring(pair, 1, CHARINDEX('/', pair)-1) ) fsym
		cross apply (select sid from tokens where symbol=substring(pair, CHARINDEX('/', pair)+1, len(pair))) tsym
	...
	where not exists (	select 1 from mem.order_book o 
						where o.exchange_id=exchange_id --and o.pair=om.pair 
						and o.is_bid=x.is_bid
						and o.id_fsym=fsym.sid
						and o.id_tsym=tsym.sid
						and o.is_bid=x.is_bid
						and o.price=x.price 
						and o.amount=x.amount)
--	order by price
